...
const internalPackagePrefix = "@southbag/code";
const installPackageName = "@southbag/code-install";
...
function createInstallerPackageJson(codingAgentPackage) {
	const packageJson = {
		...
		private: true,
		description: "Lockfile root used by the Southbag Code installer and updater.",
		dependencies: {
...
try {
	...
	if (checkOnly) {
		if (!existsSync(outputPackageJsonPath) || !existsSync(outputLockfilePath)) {
			...
			console.error("Run: bun run install-lock:coding-agent");
			process.exit(1);
		}
