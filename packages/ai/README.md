...
```bash
npm install @southbag/code-ai
```

TypeBox exports are re-exported from `@southbag/code-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@southbag/code-ai';
import { builtinModels } from '@southbag/code-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@southbag/code-ai/providers/anthropic';
import { openaiProvider } from '@southbag/code-ai/providers/openai';
import { openrouterProvider } from '@southbag/code-ai/providers/openrouter';
import { amazonBedrockProvider } from '@southbag/code-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@southbag/code-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@southbag/code-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@southbag/code-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@southbag/code-ai';
...
```typescript
import { validateToolCall, type Tool } from '@southbag/code-ai';
...
```typescript
import { builtinImagesModels } from '@southbag/code-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@southbag/code-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@southbag/code-ai';
import { openAICompletionsApi } from '@southbag/code-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@southbag/code-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@southbag/code-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@southbag/code-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@southbag/code-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@southbag/code-ai';
import { anthropicProvider } from '@southbag/code-ai/providers/anthropic';
import { openaiProvider } from '@southbag/code-ai/providers/openai';
import { googleProvider } from '@southbag/code-ai/providers/google';
...
```typescript
import { createModels } from '@southbag/code-ai';
import { anthropicProvider } from '@southbag/code-ai/providers/anthropic';
...
- `@southbag/code-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@southbag/code-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@southbag/code-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@southbag/code-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@southbag/code-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@southbag/code-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@southbag/code-ai/bedrock-provider';
...
```bash
npx @southbag/code-ai login              # interactive provider selection
npx @southbag/code-ai login anthropic    # login to specific provider
npx @southbag/code-ai list               # list available providers
```
...
The legacy flow functions remain available via the `@southbag/code-ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@southbag/code-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@southbag/code-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@southbag/code-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@southbag/code-ai`.
