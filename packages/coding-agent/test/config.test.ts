...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@southbag/code")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @southbag/code",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@southbag/code")).toBeUndefined();
		expect(getUpdateInstruction("@southbag/code")).toBe(
			"Update @southbag/code using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@southbag/code");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@southbag/code"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @southbag/code`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@southbag/code", undefined, {
			packageName: "@southbag/code",
			installSpec: "@southbag/code@1.2.3",
		});

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@southbag/code@1.2.3"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @southbag/code@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@southbag/code", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @southbag/code`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@southbag/code")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @southbag/code",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@southbag/code"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @southbag/code",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@southbag/code";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@southbag/code")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@southbag/code")).toContain("the install path is not writable");
	});
});
