...
- Never run `bun run build` or `bun test` unless requested by the user.
...
- If dep metadata changes, refresh `bun.lock` with `bun install --lockfile-only --ignore-scripts`. Keep `package-lock.json` synchronized only because the npm release artifacts are derived from it.
- If `packages/coding-agent/npm-shrinkwrap.json` needs regen, run `node scripts/generate-coding-agent-shrinkwrap.mjs` (verify with `--check` or `bun run check`). New deps with lifecycle scripts require review and an explicit allowlist entry in that script; never add one silently.
...
2. **Local smoke test**: build an unpublished release and smoke test from outside the repo (so it can't resolve workspace files):
   ```bash
   bun run release:local -- --out /tmp/southbag-code-local-release --force
   cd /tmp
...
3. **Run the release script**:
   ```bash
   PI_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 bun run release:patch    # fixes + additions
   PI_ALLOW_LOCKFILE_CHANGE=1 npm_config_min_release_age=0 bun run release:minor    # breaking changes
   ```
